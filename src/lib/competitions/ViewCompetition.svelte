...
<div class="container overflow-auto p-1">
	<table class="table table-sm align-middle p-0">
				...
				<tr class={cuts[i + 1] && display == 'Results' ? 'cutrow' : ''}>
					...
					{#each $activeComp!.children as stage, j}
						{#if display == 'Results' && j > 0 && cuts[i]?.includes($activeComp?.children[j - 1].summary.name)}
							<td class=" text-primary align-text-top b-0" colspan={stage.children.length + 1}
								>{$activeComp?.children[j - 1].summary.name} Cut</td
							>
						{:else}
							{#each stage.children as round}
