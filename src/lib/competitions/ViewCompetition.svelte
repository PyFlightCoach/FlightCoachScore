...
<div class="row-auto table-responsive p-">
			...
			{#each $activeComp!.sortCompetitors(display) as competitor, i}
				<tr>
					...
					{#each $activeComp!.children as stage}
						{#each stage.children as round}
							<ScoreCell {round} competitorID={competitor.competitor.id} bind:display={display} />
						{/each}
						<ScoreCell round={stage} competitorID={competitor.competitor.id} bind:display={display} />
					{/each}
					<ScoreCell round={$activeComp!} competitorID={competitor.competitor.id} bind:display={display} />
				</tr>
			{/each}
