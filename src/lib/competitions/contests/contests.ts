...
export async function listComps(
  ...
  include_competitors: boolean = true
	...
	return dbServer.get('/competition/list', { params }).then((comps) =>
		Promise.all(
			comps.data?.results
				.filter((c: CompListResponse) => {
				...
				.map((c: CompListResponse) =>
					loadChildren
						? ContestManager.load(c.compthing.id, c.i_am_cd, c.i_am_competitor, c.i_can_upload_to)
						: new ContestManager(
								c.compthing,
								undefined,
								c.i_am_cd,
...
export const contestActions = ['View', 'Enter', 'Select', 'Manage'];
