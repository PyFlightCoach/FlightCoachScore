<script lang="ts">
	import type {
		CompThingCreateUpdate,
		CompThingSummary,
		Thing,
	} from '$lib/competitions/compInterfaces';
	import CheckInput from '$lib/components/CheckInput.svelte';
	import TextInput from '$lib/components/TextInput.svelte';
	import { dbServer } from '$lib/api';
	
	import { faVersion } from '$lib/stores/shared';
	import { activeComp, updateCDComps } from '$lib/stores/contests';
	
	let {
		parentID,
		whatAmI,
		thing,
    isMyComp = false,
		oncreated = () => {}
	}: {
		parentID?: string | undefined;
		whatAmI: Thing;
		thing: CompThingSummary;
    isMyComp?: boolean;
		oncreated?: () => void;
	} = $props();

	let name: string | undefined = $state(thing.name);
	let comment: string | undefined = $state(thing.comment);
	let allowSelfAdd: boolean = $state(thing.add_rules?.cd_and_self_add || false);

	let compnormalisation = thing.result_rules?.raw_score
		? 'raw'
		: thing.result_rules?.normalise_average_to_n
		? 'average'
		: 'best';

	let normalisation: string = $state(compnormalisation);
	let normalise_to: number | undefined = $state(thing.result_rules?.normalise_best_to_n || thing.result_rules?.normalise_average_to_n || 1000);
	let progressTopN: number | undefined = $state(thing.result_rules?.progress_top_n);
	let hide_results: boolean = $state(thing.hide_results || false);
	let uploadWithin: number | undefined = $state(thing.flight_rules?.upload_within_n_hours);
	let flownWhilstOpen: boolean = $state(thing.flight_rules?.flown_whilst_open || false);
	let uploadWhilstOpen: boolean = $state(thing.flight_rules?.upload_whilst_open || false);
	let finalised: boolean = $state(thing.flight_rules?.finalised || false);

	function hasChanged(val1: any, val2: any) {
		return val1 != val2 ? 'bg-warning' : '';
	}

	function createUpdateRequest() {
		let data = {
			name,
			comment,
			result_rules: {
				normalise_best_to_n: normalisation === 'best' ? normalise_to : undefined,
				normalise_average_to_n: normalisation === 'average' ? normalise_to : undefined,
				raw_score: normalisation === 'raw'
			},
			hide_results
		} as CompThingCreateUpdate;

		if (whatAmI == 'Competition' || whatAmI == 'Stage') {
			data.add_rules = {
				cd_only: !allowSelfAdd,
				cd_and_self_add: allowSelfAdd
			};
		}

		if (whatAmI == 'Stage') {
			data.flight_rules = {
				upload_within_n_hours: uploadWithin,
				flown_whilst_open: flownWhilstOpen,
				upload_whilst_open: uploadWhilstOpen,
				finalised: finalised
			};
		}

		if (parentID) {
			data.parent_id = parentID;
		}
		return data;
	}

</script>

<small
	><ul class="list-unstyled">
		<li>id={thing?.id}</li>
    {#if parentID}
		<li>parent_id={parentID}</li>
    {/if}
		<li>whatAmI={whatAmI}</li>
		<li>open={thing?.is_open_now}</li>
		<li>hide result={thing?.hide_results}</li>
    <li>n competitors={thing?.competitors?.length}</li>
    {#each thing?.directors || [] as director, i} 
      <li>Director{i>0 ? i+1 : ""}: {director.name}</li>
    {/each}
	</ul></small
>

<TextInput name="Name" bind:value={name} classappend={hasChanged(name, thing?.name)} disabled={!isMyComp}/>
<TextInput name="Comment" bind:value={comment} classappend={hasChanged(comment, thing?.comment)} disabled={!isMyComp}/>
<hr />
<small>Result Rules</small>

<div class="row p-2 {hasChanged(normalisation, compnormalisation)}">
	<label class="col col-form-label" for="normOption">Normalisation:</label>
	<select class="col col-form-input form-select" id="normOption" bind:value={normalisation} disabled={!isMyComp}>
		<option value="raw">Raw Score</option>
		<option value="best">Best Score</option>
		<option value="average">Average Score</option>
	</select>
</div>
{#if normalisation != 'raw'}
	<div class="row p-2 {hasChanged(normalise_to, thing?.result_rules?.normalise_best_to_n || 1000)}">
		<label class="col col-form-label" for="normTo">Normalise to:</label>
		<input class="col form-control" type="number" id="normTo" bind:value={normalise_to} disabled={!isMyComp}/>
	</div>
{/if}
<div class="row p-2 {hasChanged(progressTopN, thing?.result_rules?.progress_top_n)}">
	<label class="col col-form-label" for="progressTopN">Progress top n:</label>
	<input class="col form-control" type="number" id="progressTopN" bind:value={progressTopN} disabled={!isMyComp}/>
</div>

{#if whatAmI == 'Competition' || whatAmI == 'Stage'}
	<hr />
	<small>Add Rules</small>

	<CheckInput
		name="Allow pilots to submit their own flights"
		bind:checked={allowSelfAdd}
		classappend={hasChanged(allowSelfAdd, thing?.add_rules?.cd_and_self_add || false)}
    disabled={!isMyComp}
	/>
{/if}
{#if whatAmI == 'Stage'}
	<hr />
	<small>Flight Rules</small>

	<CheckInput
		name="Must boot whilst round is open"
		bind:checked={flownWhilstOpen}
		classappend={hasChanged(flownWhilstOpen, thing?.flight_rules?.flown_whilst_open || false)}
    disabled={!isMyComp}
	/>
	<CheckInput
		name="Must upload whilst round is open"
		bind:checked={uploadWhilstOpen}
		classappend={hasChanged(uploadWhilstOpen, thing?.flight_rules?.upload_whilst_open || false)}
    disabled={!isMyComp}
	/>
	<div class="row p-2 {hasChanged(uploadWithin, thing?.flight_rules?.upload_within_n_hours)}">
		<label class="col col-form-label" for="normTo">upload within (hours from boot):</label>
		<input class="col form-control" type="number" id="normTo" bind:value={uploadWithin} disabled={!isMyComp}/>
	</div>
{/if}
{#if whatAmI == 'Round'}
	<CheckInput
		name="hide results"
		bind:checked={hide_results}
		classappend={hasChanged(hide_results, thing?.hide_results || false)}
    disabled={!isMyComp}
	/>
{/if}

{#if isMyComp}
<div class="row p-2">
	<button
		class="col btn btn-outline-primary"
		onclick={() => {
			dbServer.get(`competition/${$activeComp.id}`).then((res) => {
				$activeComp = res.data;
			});
		}}>Reset</button
	>
	<button
		class="col btn btn-outline-primary"
		onclick={() => {
			if (!thing?.id) {
				dbServer
					.post('competition', createUpdateRequest())
					.then((res) => {
						updateCDComps();
						$activeComp = res.data;
					})
					.then(oncreated);
			} else {
				dbServer.patch(`competition/${thing.id}`, createUpdateRequest()).then((res) => {
					$activeComp = res.data;
				});
			}
		}}
	>
		Save
	</button>
</div>
{/if}