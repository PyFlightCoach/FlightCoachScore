<script lang="ts">
	...
	let { competition, onadded = () => {} }: { competition: ContestManager; onadded?: () => void } = $props();
...
{#if !results}
	<small>Enter pilots name or email address</small>
	<TextInput
		name={'pilot'}
	...
	<div class="row p-2">
		<button
			...
			onclick={() => {
				dbServer
					.post('users/search', { name_or_email })
					.then((res) => {
						results = res.data;
					})
					.catch((e) => {
...
{:else}
  ...
  <div class="row p-2">
    <button
      class="col btn btn-outline-primary"
      onclick={() => {results = undefined; selected=undefined;}}
    >
      Back
    </button
    >
