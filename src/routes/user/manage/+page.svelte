<script lang="ts">
	...
	async function _patch(item: any) {
		if (
			confirm(
				`Are you sure you want to ${Object.keys(item)[0]} ${Object.values(item)[0]} user ${showID}?`
			)
		) {
                ...
                <td colspan="5">
                  <div class="row">
                    <div class="col">
                      ...
                      <p><strong>Created:</strong> {new Date(u.joined_when).toLocaleString()}</p>
                      <p><strong>is CD:</strong> {u.is_cd}</p>
                      <p><strong>is Superuser:</strong> {u.is_superuser}</p>
                      <p><strong>is Active:</strong> {u.is_active}</p>
                      <p><strong>is Verified:</strong> {u.is_verified}</p>
                    </div>
                  </div>
                  </td>
              </tr>
								...
								<td colspan="5">
									{#if u.is_active}
										...
										{#if u.is_superuser}
											<button
												...
												class="btn btn-primary"
												on:click={() => {
													_patch({ is_superuser: false });
												}}>Remove S-User</button
											>
										{:else}
											<button
												type="button"
									...
									{:else}
										...
										{:else}
											...
											>
											<button type="button" class="btn btn-primary" on:click={_delete}
												>Yes DELETE!</button
											>
										{/if}
									{/if}
