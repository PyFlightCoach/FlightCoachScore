<script lang="ts">
	...
	function activate_man(id: number, page: string) {
		$selManID = id;
		goto(resolve('/flight/results/manoeuvre') + `/${page}`);
	}
</script>
...
<tr>
	...
	<td role="button">
		<button
			class="btn btn-link p-0 link-dark"
			title="View {$ma?.name}"
			onclick={() => activate_man(id, '')}
		>
			{$ma?.name}
		</button>
	</td>
	...
	{#if scores}
		<td class="text-center" style:background-color={colscale(intra, 6, colours)}>
			<button
				class="btn btn-link p-0 link-dark {isRunning ? 'disabled' : ''}"
				title="{$ma?.name} intra information"
				onclick={() => activate_man(id, 'intra')}
			>
				{intra.toFixed(2)}
			</button>
		</td>
		<td class="text-center" style:background-color={colscale(inter, 6, colours)}>
			<button
				class="btn btn-link p-0 link-dark {isRunning ? 'disabled' : ''}"
				title="{$ma?.name} inter information"
				onclick={() => activate_man(id, 'inter')}
			>
				{inter.toFixed(2)}
			</button>
		</td>
		<td class="text-center" style:background-color={colscale(positioning, 6, colours)}>
			<button
				class="btn btn-link p-0 link-dark {isRunning ? 'disabled' : ''}"
				title="{$ma?.name} positioning information"
				onclick={() => activate_man(id, 'positioning')}
			>
	...
	{#if scores}
		<td class="text-center" style:background-color={colscale(10 - score, 10, colours)}>
			{score.toFixed(1)}
		</td>
	{:else}
		<td>-</td>
